...
// barra inteira é o volume de leads e o trecho escuro é quem matriculou, em
// tons de verde; a matrícula e a conversão também ficam em verde, como nas
// tabelas de pessoas e de professores. No celular, rótulo em cima e barra
// embaixo.
...
const VOLUME = 'bg-emerald-200 dark:bg-emerald-500/30';
const ENROLLED = 'bg-success dark:bg-[#0E9F6E]';
...
function Bar({ row, max, tip, className, radius }) {
  return (
    <ChartMark tip={tip} className={cn('relative block bg-muted', radius, className)}>
      ...
      <i className={cn('absolute inset-y-0 left-0', radius, ENROLLED)} style={{ width: widthOf(row.enrolled, max) }} />
    </ChartMark>
  );
}

export function ChannelTable({ rows, read }) {
  ...
  const legend = (
    <div className="hidden flex-none items-center gap-3 sm:flex">
      ...
      <Swatch className={cn('w-[18px] rounded', ENROLLED)}>matricularam</Swatch>
    </div>
  );
  return (
        ...
        <div className="px-[18px] pb-4 pt-2">
          ...
          {list.map((r) => (
            <div key={r.name} className={cn(GRID, 'hidden h-9 border-t md:grid', RULE)}>
              ...
              <span className={cn('num text-right text-[12.5px] font-bold', r.conv != null && GREEN_TEXT)}>{convLabel(r)}</span>
            </div>
          ))}
            ...
            {list.map((r) => (
              <div key={r.name}>
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  <span className="num flex-none text-[11.5px] text-muted-foreground">
                    {`${fmtNum(r.leads)} ${r.leads === 1 ? 'lead' : 'leads'} · `}
                    <span className={cn('font-semibold', r.conv != null && GREEN_TEXT)}>{convLabel(r)}</span>
                  </span>
                </div>

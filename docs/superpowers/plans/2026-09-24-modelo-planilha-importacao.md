...
- **Task 4, depois:** o download espera um segundo antes de soltar o endereço do arquivo (Safari), e o teste do XML não depende da ordem dos atributos. O pedaço `exceljs` do build só aparece depois da Task 7, quando a tela passa a importar o gerador; a conferência do build foi para a Task 7.
- **Task 7, depois:** a escolha de plano em Ajustes só vale enquanto o nome continua fora do catálogo e o plano ainda existe; o seletor de plano ganhou nome acessível; "Gerando…" com reticências de um caractere.
- **Task 9:** o item do CLAUDE.md sobre as datas diz que o limite vai como `Date` em meia-noite UTC, e que só o teste do XML prova isso; o item sobre as marcas do importado diz exatamente o que `isImportedContract`, `isImportCreatedLead` e `isImportedEnrollment` olham.
- **Revisão final:** a lista de donos do modelo e da importação só traz quem pode ser dono de lead (mesmo filtro do cadastro de cliente); as regras de plano saíram da tela para `unmatchedPlanNames` e `livePlanMap`, em `clientImport.js`, com testes; entrou um teste de que todo nome escolhido na lista casa na importação; `parseRow` avisa "Sem plano"; a importação lê a aba Clientes pelo nome; e o texto de ajuda fala só das colunas com nota e pede para não renomear a aba.

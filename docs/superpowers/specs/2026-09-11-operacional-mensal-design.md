...
- Trancaram e destrancaram contam a pausa do contrato, e não a transição da ponte: pessoas com uma pausa que começou, ou terminou, em M. Contrato paralelo é permitido (decisão de 14/09/2026). Quem tranca um de dois contratos segue na base pelo outro, então o passo da ponte não muda, mas o trancamento aparece no card, que mostra o saldo desses eventos. As pausas vêm do `pauseHistory`, que a reativação passa a gravar a partir desta entrega. Nos contratos antigos, a pausa é reconstruída por `resumedAt − pausedDaysTotal`, e várias pausas antigas viram uma só (limitação conhecida).
